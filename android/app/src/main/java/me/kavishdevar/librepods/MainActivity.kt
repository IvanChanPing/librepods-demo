...
class MainActivity : ComponentActivity() {
    companion object {
        init {
            if (!BuildConfig.DEMO_MODE && XposedState.isAvailable && XposedState.bluetoothScopeEnabled) {
...
fun Main() {
    ...
    LaunchedEffect(Unit) {
        if (BuildConfig.DEMO_MODE) {
            airPodsViewModel.initDemo(
                sharedPreferences = sharedPreferences,
                appContext = context.applicationContext,
            )
        } else if (BuildConfig.PLAY_BUILD) {
    ...
    val onboardingComplete = BuildConfig.DEMO_MODE || sharedPreferences.getBoolean("onboarding_complete", false)

    val releaseNotesShownPrefKey = "release_notes_shown_${BuildConfig.VERSION_NAME.removeSuffix("-debug").removeSuffix("-play")}"
    val releaseNotesShown = BuildConfig.DEMO_MODE || sharedPreferences.getBoolean(releaseNotesShownPrefKey, false)
    ...
    if (onboardingComplete && !BuildConfig.DEMO_MODE) {
        bindService()
    }
