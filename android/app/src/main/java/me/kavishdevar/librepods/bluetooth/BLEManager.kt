...
class BLEManager(private val context: Context) {
    ...
    private fun processScanResult(result: ScanResult) {
        try {
            ...
            val parsedStatus = if (decryptedData != null && decryptedData.size == 16) {
                parseProximityMessageWithDecryption(address, result.rssi, manufacturerData, decryptedData)
            } else {
                parseProximityMessage(address, result.rssi, manufacturerData)
            }
    ...
    private fun parseProximityMessageWithDecryption(address: String, rssi: Int, data: ByteArray, decrypted: ByteArray): AirPodsStatus {
...
//        val flagsCase = data[7].toInt() and 0xFF
    ...
    private fun parseProximityMessage(address: String, rssi: Int, data: ByteArray): AirPodsStatus {
        val paired = data[2].toInt() == 1
        val modelId = ((data[3].toInt() and 0xFF) shl 8) or (data[4].toInt() and 0xFF)
        val model = modelNames[modelId] ?: "Unknown ($modelId)"
