...
class AppSettingsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        loadSettings()
        if (!BuildConfig.DEMO_MODE) observeBilling()
    ...
    private fun loadSettings() {
        ...
        _uiState.update { currentState ->
            currentState.copy(
                ...
                vendorIdHook = if (BuildConfig.DEMO_MODE) true else xposedRemotePref.getBoolean("vendor_id_hook", false),
                isPremium = if (BuildConfig.DEMO_MODE) true else currentState.isPremium,
                connectionSuccessful = BuildConfig.DEMO_MODE || sharedPreferences.getBoolean("connection_successful", false),
    ...
    fun setVendorIdHook(enabled: Boolean) {
        if (!BuildConfig.DEMO_MODE) xposedRemotePref.putBoolean("vendor_id_hook", enabled)
