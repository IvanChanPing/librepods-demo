...
fun AudioSettings(
    ...
    if (adaptiveVolumeCapability || conversationalAwarenessCapability || loudSoundReductionCapability || adaptiveAudioCapability || customEqCapability) {
        StyledList(title = stringResource(R.string.audio)) {
            StyledListItem(
                name = "Spatial Audio",
                description = spatialAudioMode.label,
                onClick = navigateToSpatialAudio,
            )
