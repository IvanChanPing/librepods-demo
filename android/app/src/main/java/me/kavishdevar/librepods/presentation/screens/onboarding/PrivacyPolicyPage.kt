...
fun PrivacyPolicyPage(
    ...
    Box(
        ...
        Column(
            ...
            Text(
                text = "Last updated: 11 August 2026",
            ...
            Text(
                text = "LibrePods does not sell personal information or use advertising, profiling, or tracking services. This debug build includes optional support diagnostics that remain off unless you explicitly enable them.",
                style = MaterialTheme.typography.bodyMedium
            )

            Text(
                text = "Information remains on your device except when you explicitly enable support diagnostics, choose to contact me, create a GitHub issue from the app, or make a purchase or sponsorship through a third-party platform.",
                style = MaterialTheme.typography.bodyMedium
            )
            ...
            Text(
                text = "Optional Support Diagnostics",
                style = MaterialTheme.typography.titleLarge
            )

            Text(
                text = "If you opt in on the separate support-diagnostics prompt, this debug build automatically sends only the event name and time, LibrePods version, Android SDK level, and device model over HTTPS to this build's private support collector. These events are used only to diagnose the features in this build and are deleted after no more than 14 days.",
                style = MaterialTheme.typography.bodyMedium
            )

            Text(
                text = "Diagnostics never include Apple or OpenBubbles credentials, account identifiers, AirPods identifiers, locations, heart-rate values, audio, messages, contacts, purchases, or Bluetooth protocol payloads. The collector does not store network addresses. You can decline the prompt or turn diagnostics off later in App Settings; turning it off discards queued events.",
                style = MaterialTheme.typography.bodyMedium
            )
            ...
            Text(
                text = "This GitHub issue information is only submitted if you choose to create the issue. It is separate from the optional support diagnostics described above.",
                style = MaterialTheme.typography.bodyMedium
            )
